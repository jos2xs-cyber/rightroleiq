...
var onRequestPost = /* @__PURE__ */ __name(async ({ request, env }) => {
  try {
    ...
    try {
      ...
      const isRateLimit = groqError?.status === 429 || groqError?.message?.toLowerCase().includes("rate limit") || groqError?.message?.toLowerCase().includes("quota");
...
${prompt}` }] }],
        ...
        const googleError = await googleResponse.json().catch(() => ({}));
        const googleIsRateLimit = googleResponse.status === 429 || googleError?.error?.message?.toLowerCase().includes("quota") || googleError?.error?.message?.toLowerCase().includes("rate limit");
...
// ../.wrangler/tmp/pages-wJf7By/functionsRoutes-0.18839297751898187.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-8J85AX/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-8J85AX/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.8620920000678896.mjs.map
